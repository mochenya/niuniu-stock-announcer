...
class _CninfoClient(Protocol):
    def query_announcements(self, market: str, **kwargs: object) -> Any: ...
...
def _create_client() -> _CninfoClient:
...
class CninfoAnnouncementService:
    """复用一个按需 CNInfo client 执行查询和 PDF 下载。"""

    def __init__(
        self, client_factory: Callable[[], _CninfoClient] = _create_client
    ) -> None:
        ...
        self._client: _CninfoClient | None = None

    @property
    def provider_key(self) -> ProviderKey:
        """返回 CNInfo Provider key。"""
        return "cninfo"

    def query(self, query: AnnouncementQuery) -> ProviderQueryResult:
        """执行 CNInfo 查询并逐条隔离确定的 mapper 失败。

        Args:
            query: 已校验且仅含一种发现条件的查询。
        ...
        if query.stock_code is not None:
            kwargs["stock"] = query.stock_code
        else:
            kwargs["searchkey"] = query.search_keyword
        result = self._require_client().query_announcements(query.exchange, **kwargs)
        mapped = []
        errors = []
        for index, item in enumerate(result.response.announcements):
            try:
                mapped.append(map_cninfo_announcement(item, query))
        ...
        if query.limit is not None and len(mapped) > query.limit:
            mapped = mapped[: query.limit]
            has_more = True
    ...
    def download_pdf(
        ...
        Returns:
            SDK 实际写入的 PDF 路径。
        """
        if announcement.provider_key != "cninfo":
            raise ValueError("CNInfo service 不能下载其他 Provider 公告")
    ...
    def close(self) -> None:
        """关闭已创建的 CNInfo client；尚未使用时保持无副作用。"""
        if self._client is not None:
            self._client.close()
            self._client = None

    def __enter__(self) -> CninfoAnnouncementService:
        return self

    def __exit__(self, *_args: object) -> None:
        self.close()

    def _require_client(self) -> _CninfoClient:
