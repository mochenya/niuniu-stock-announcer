...
def _create_client() -> _SseClient:
    from sse_announcement.client import SSEAnnouncementClient

    return SSEAnnouncementClient()


class SseAnnouncementService:
    """复用一个按需 SSE client 执行查询和 PDF 下载。"""

    def __init__(
        self, client_factory: Callable[[], _SseClient] = _create_client
    ) -> None:
        ...
        self._client: _SseClient | None = None
    ...
    def query(self, query: AnnouncementQuery) -> ProviderQueryResult:
        """执行 SSE 查询并逐条隔离确定的 mapper 失败。

        Args:
            query: 已校验且 exchange 必须为 `sh` 的查询。
        ...
        for index, item in enumerate(result.response.announcements):
            try:
                mapped.append(map_sse_announcement(item, query))
    ...
    def download_pdf(
        ...
        Returns:
            SDK 实际写入的 PDF 路径。
        """
        if announcement.provider_key != "sse":
            raise ValueError("SSE service 不能下载其他 Provider 公告")
    ...
    def close(self) -> None:
        """关闭已创建的 SSE client；尚未使用时保持无副作用。"""
        if self._client is not None:
            self._client.close()
            self._client = None

    def __enter__(self) -> SseAnnouncementService:
        return self

    def __exit__(self, *_args: object) -> None:
        self.close()

    def _require_client(self) -> _SseClient:
